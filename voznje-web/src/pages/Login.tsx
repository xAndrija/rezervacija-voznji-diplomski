...
export default function Login() {
  ...
  const posaljiFormu = async (e: React.FormEvent) => {
    ...
    try {
      ...
      ulogujSe(podaci.token, podaci.korisnik);
      navigate('/voznje');
    } catch (err: any) {
    ...
    <div className="min-h-screen flex">
      <div className="hidden md:flex md:w-1/2 bg-text relative overflow-hidden flex-col justify-between p-12">
        ...
        <div className="relative">
          ...
          <ul className="space-y-3 mt-8">
            {[
              'Pronađi saputnike na svojoj ruti',
              'Podeli troškove goriva umesto da putuješ sam',
              'Rezerviši mesto u par klikova',
            ].map((tekst) => (
      ...
      <div className="flex-1 flex items-center justify-center bg-bg px-4 py-10">
        <div className="w-full max-w-sm">
          ...
          <h1 className="font-display font-semibold text-2xl text-text mb-1">Dobrodošao nazad</h1>
          <p className="text-sm text-text-soft mb-6">Prijavi se da nastaviš putovanje</p>
          ...
          <p className="text-sm text-text-soft mt-5 text-center">
            Nemaš nalog?{' '}
            <Link to="/registracija" className="text-accent font-medium">
              Registruj se
            </Link>
          </p>
        </div>
